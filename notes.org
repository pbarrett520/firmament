...
* log
** effects in gui
hacked it together, here's notes:
...
- [ ] propagating the draw flag is a stupid hack that's just for this, not sure
  better way to do it but surely there is something
  - NOTE: a better way to do this is probably appending imgui metadata onto
    tables -- this is kind of ugly, because you probably don't want to save
    these out. but it's the only way to get something like
    [field_add][none][field_add]
- [X] print effect type or a name if one is given instead of table index
- [ ] bind effect types between c and lua
- [ ] master list of effect types also
** patrick 2
