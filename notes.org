...
- [ ] fix editor layout flashing

quick dogfood notes:
- typing into the selected view, having to press enter is annoying
- rename Empty to ChoiceRepeat
- instead of the backward node connections, just have a Return node or something
- easy way to set prism's name as known without duplicating nodes
- multiselect
