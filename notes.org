...
- make the pane a bit bigger
- enter on value does not work
- if a node has no effects, you have to click off and click on to regenerate the
...
- print effect type or a name if one is given instead of table index
